...
last_9_vars <- final[, (ncol(final)-9):ncol(final)]

# Use the existing correlation matrix
cor_matrix <- cor(final[, (ncol(final)-9):ncol(final)])
```
...
for (j in 1:ncol(last_9_vars)) {
    ...
    for (i in 1:(nrow(last_9_vars) - 1)) {
        # Calculate the growth rate for the current element
        growth_rate <- (last_9_vars[i+1, j] - last_9_vars[i, j]) / last_9_vars[i, j] * 100
        
        # Apply the weighted growth rate using the existing correlation matrix
        weighted_growth_rate <- growth_rate * normalized_weights[j, j]
        ...
        # Calculate the t+1 value for this specific element
        t_plus_1_estimates[i, j] <- last_9_vars[i, j] * (1 + (weighted_growth_rate + epsilon) / 100)
    }
}
...
my_data <- data.frame(
    ...
    expected_K_hat_next=rep(1,111),
expected_Pi_hat_next = rep(1, 111),
expected_Q_hat_next = rep(1, 111),
expected_R_hat_next = rep(1, 111),
expected_W_hat_next = rep(1, 111),
expected_L_hat_next = rep(1, 111),
expected_C_hat_next = rep(1, 111),
expected_I_hat_next = rep(1, 111),
...
my_data$C_hat = final_with_predictions$Consumption_growth_t_plus_1
my_data$C_hat_lag1 = final_with_predictions$Consumption_growth

my_data$I_hat = final_with_predictions$Investment_growth_t_plus_1
my_data$I_hat_lag1 = final_with_predictions$Investment_growth

my_data$Y_hat = final_with_predictions$Output_growth_t_plus_1
my_data$Y_hat_lag1 = final_with_predictions$Output_growth

my_data$Pi_hat = final_with_predictions$Inflation_t_plus_1
my_data$Pi_hat_lag1 = final_with_predictions$Inflation

my_data$R_hat = final_with_predictions$Ffr_t_plus_1
my_data$R_hat_lag1 = final_with_predictions$Ffr

my_data$L_hat = final_with_predictions$Hours_t_plus_1
...
my_data$I_hat[i] = (1 / (1 + Beta)) * my_data$I_hat_lag1[i] +   
  ...
  # Calculate the parts that do not include expected_I_hat_next[i]
isolated_part <- my_data$I_hat[i] -
                 ((1 / (1 + Beta)) * my_data$I_hat_lag1[i]) -
                 ((1/phi_uppercase) / (1 + Beta)) * my_data$Q_hat[i] -
                 my_data$epsilon_I[i]
 ...
 my_data$K_hat[i]=(1-tao)*my_data$K_hat_lag1[i]+tao*my_data$I_hat_lag1[i]
   ...
   # Recalculate all parts except part1w
...
data_list <- list(
  ...
  epsilon_W = my_data$epsilon_W,
   expected_Pi_hat_next = my_data$expected_Pi_hat_next,
expected_Q_hat_next = my_data$expected_Q_hat_next,
expected_I_hat_next=my_data$expected_I_hat_next,
expected_R_hat_next = my_data$expected_R_hat_next,
expected_W_hat_next = my_data$expected_W_hat_next,
expected_L_hat_next = my_data$expected_L_hat_next,
expected_C_hat_next = my_data$expected_C_hat_next,
...
lengths <- sapply(list(
 ...
 my_data$r_k_star,
  ...
  final$Inflation,
  final$Ffr,
 my_data$expected_I_hat_next,
  my_data$expected_Pi_hat_next,
 my_data$expected_Q_hat_next,
 my_data$expected_R_hat_next,
 my_data$expected_W_hat_next,
my_data$expected_L_hat_next,
my_data$expected_C_hat_next,
  my_data$K_hat,
  my_data$Q_hat,
  my_data$pi_bar,
...
# Identify vectors shorter than T
short_vectors <- names(lengths)[lengths < T]

# Check if there are any short vectors and stop execution with a message
if (length(short_vectors) > 0) {
  stop(paste("The following vectors are shorter than", T, ":", paste(short_vectors, collapse=", ")))
}
...
data_list <- list(
  ...
  expected_I_hat_next=my_data$expected_I_hat_next[1:T],
  expected_Pi_hat_next = my_data$expected_Pi_hat_next[1:T],
expected_Q_hat_next = my_data$expected_Q_hat_next[1:T],
expected_R_hat_next = my_data$expected_R_hat_next[1:T],
expected_W_hat_next = my_data$expected_W_hat_next[1:T],
expected_L_hat_next = my_data$expected_L_hat_next[1:T],
expected_C_hat_next = my_data$expected_C_hat_next[1:T],
...
data {
  ...
  vector[T]expected_Q_hat_next;
  vector[T]expected_I_hat_next;
  vector[T]expected_R_hat_next;
  vector[T]expected_W_hat_next;  
...
parameters {
 ...
 transformed parameters {
  ...
  vector[T]expected_Q_hat_next_shocked;
  vector[T]expected_I_hat_next_shocked;
  vector[T]expected_R_hat_next_shocked;
  vector[T]expected_W_hat_next_shocked;   
  ...
  expected_Q_hat_next_shocked=expected_Q_hat_next;
  expected_I_hat_next_shocked=expected_I_hat_next;
  expected_R_hat_next_shocked=expected_R_hat_next;
  expected_W_hat_next_shocked=expected_W_hat_next;   
 ...
 for (t in shock_time:min(shock_time + 4, T)) {
  ...
  for (t in (shock_time + 1):T) {
    ...
    real part2 = (1 / (1 + h)) *expected_C_hat_next_shocked[t] ; 
    real part3 = ((sigma_c - 1) / (sigma_c * (1 + lambda_w) * (1 + h))) * (hours_shocked[t-1] -expected_L_hat_next[t]);
    ...
    q_shocked[t] = -(ffr_shocked[t] - inflation_shocked[t-1]) + ((1-tao) / (1 - tao + ffr_shocked[t])) * expected_Q_hat_next_shocked[t] +(ffr_shocked[t] / (1 - tao + ffr_shocked[t])) * ffr_shocked[t-1] + epsilon_Q[t];
    
    investment_growth_shocked[t] = (1 / (1 + Beta)) * investment_growth_shocked[t-1] + (Beta / (1 + Beta)) *expected_I_hat_next_shocked[t] + ((1/phi_uppercase)/(1+Beta)) * q_shocked[t] + epsilon_I[t];
  ...
  real parta = (Beta / (1 + Beta * gamma_pi)) * (expected_Pi_hat_next_shocked[t] - inflationtaget_shocked[t]);
   ...
   inflation_shocked[t] = parta + partb + partc + parte +  inflationtaget_shocked[t];
    ...
    real part1w = (Beta / (1 + Beta)) *expected_W_hat_next_shocked[t] ;
...
model {
  ...
  for (t in 2:T)  {
    ...
    real part3 = ((sigma_c - 1) / (sigma_c * (1 + lambda_w) * (1 + h))) * (hours_vector[t] - expected_L_hat_next[t]);
 ...
 q[t] ~ normal(-(ffr_vector[t] - inflation_vector[t]) +
      ((1-tao) / (1 - tao + ffr_vector[t])) *expected_Q_hat_next[t]+
    (r_k_star[t] / (1 - tao + ffr_shocked[t])) * ffr_vector[t-1]+epsilon_Q[t],5.701086);
    
    investment_growth_vector[t] ~ normal((1 / (1 + Beta)) * investment_growth_vector[t-1]     +(Beta / (1 + Beta)) *expected_I_hat_next[t] +
...
// Breaking down the calculation into parts
 real parta = (Beta / (1 + Beta * gamma_pi)) * (expected_Pi_hat_next[t]-inflationtaget[t]);
...
// Combine parts
  inflation_vector[t]~  normal(parta + partb + partc + parte +  inflationtaget[t],0.11154431);
     ...
     real part1w = (Beta / (1 + Beta)) *expected_W_hat_next[t] ;
 ...
 generated quantities {
  ...
  vector[T] expected_Q_hat_next_shocked_output;
  vector[T] expected_I_hat_next_shocked_output;
  vector[T] expected_R_hat_next_shocked_output;
  vector[T] expected_W_hat_next_shocked_output;   
  ...
  expected_Q_hat_next_shocked_output = expected_Q_hat_next_shocked;
  ...
  expected_W_hat_next_shocked_output = expected_W_hat_next_shocked;
