...
parameters {
 vector[T] expected_Pi_hat_next;
  ...
  real<lower=0.853, upper=1.3> sigma_c;
  real<lower=0.35, upper=0.7> h;
...
real part1_inflation = (Beta / (1 + Beta * gamma_pi)) * (expected_Pi_hat_next[t] - inflationtarget_shocked[t]);
real part2_inflation = (gamma_pi / (1 + Beta * gamma_pi)) * (inflation_shocked[t-1] - inflationtarget_shocked[t]);
real part3_adjustment = (1 / (1 - Beta * gamma_pi)) * (((1 - Beta * zeta_ps) * (1 - zeta_ps)) / zeta_ps);
real part4_inflation = alpha * ffr_shocked[t] + (1 - alpha) * wage_growth_shocked[t] - epsilon_pitwo[t] - (1 - alpha) * gamma_w;
real part5_inflation = epsilon_PI[t] + inflationtarget_shocked[t];

// Final inflation shocked equation
inflation_shocked[t] = part1_inflation + part2_inflation + part3_adjustment * part4_inflation + part5_inflation;


real part1_wage= (Beta / (1 + Beta)) *expected_W_hat_next[t] ;
real part2_wage= (1 / (1 + Beta)) *  wage_growth_shocked[t-1];
real part3_inflation = (Beta / (1 + Beta)) * (inflationtarget_shocked[t-1] -  inflation_shocked[t]);
real  part4_inflation_wage = ((1 + (Beta * gamma_w)) / (1 + Beta)) * (inflationtarget_shocked[t-1] - inflation_shocked[t]);
real part5w = (gamma_w / (1 + Beta)) * (inflationtarget_shocked[t-1] -inflation_shocked[t]);
real adjustment_factor = (1/(1+Beta))*((1 - (Beta * zeta_ws) * (1 - zeta_ws)) / (1 + ((1 - lambda_w) * sigma_L) / lambda_w) * zeta_ws);
real complex_part = adjustment_factor * (wage_growth_shocked[t-1] - (sigma_L * hours_shocked[t]) - (1 / (1 - h)) * (consumption_growth_shocked[t] - (h * consumption_growth_shocked[t-1])) + epsilon_L[t])+epsilon_W[t]; 
wage_growth_shocked[t] = part1_wage + part2_wage + part3_inflation - part4_inflation_wage - complex_part ;
...
model {
  ...
  mu_W_hat ~ normal(0.372847, sigma_W_hat);
 ...
 q[t] ~ normal(-(ffr_vector[t] - inflation_vector[t]) +
   ...
   hours_vector[t]~ normal(-wage_growth_vector[t] + (1+psi)*ffr_vector[t]+ kapital[t-1],0.7029192);
