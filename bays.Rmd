...
calculate_next_period_with_average_diff <- function(data) {
  ...
  diffs <- diff(tail(data, 4))
  
  # Calculate the average difference
  avg_diff <- mean(diffs)
  
  # Predict the next period by adding the rage difference to the last data point
  next_period <- tail(data, 1)*.7
...
my_data <- data.frame(
    ...
    W_hat = rep(3, 204),
    I_hat = rep(35, 204),
     C_hat_lag1 = rep(333,204),  # Lagged values for the firstrhow would be NA
    I_hat_lag1 = rep(35, 204),
     expected_I_hat_next=rep(35,204),
     expected_W_hat_next=rep(3,204),
    K_hat=rep(0,204),
    Pi_hat=rep(1,204),
    Y_hat=rep(2000,204),
    R_hat = rep(7,204),
    ...
    epsilon_W = rep(1, 204),
    epsilon_R = rep(1, 204),
    epsilon_Pi=rep(1,204),
    epsilon_L=rep(1,204),
    epsilon_G=rep(1,204),
    
     epsilon_pitwo=rep(1,204),
    expected_K_hat_next=rep(0,204),
    expected_L_hat_next=rep(7,204),
    expected_C_hat_next=rep(333,204),
    expected_Q_hat_next = rep(10, 204),
    expected_Pi_hat_next=rep(1,204),
    expected_R_hat_next = rep(7, 204),
     expected_Y_hat_next = rep(2000, 204),
   # Lagged values for the firstrhow would be NA
    Q_hat_lag1 = rep(1, 204),
    W_hat_lag1=rep(3,204),
     R_hat_lag1 = rep(7,204),
    L_hat_lag1 = rep(1,204),
    Y_hat_lag1=rep(1,204),
    YP_hat_lag1=rep(1,204),
    Pi_bar_lag1=rep(1,204),
     Pi_hat_lag1=rep(1,204),
    K_hat_lag1 = rep(0,204)
)
my_data$YP_hat=(1+(my_data$Pi_hat/100))*my_data$Y_hat
```
```{r}
...
for (i in 6:204) {
    ...
    my_data$YP_hat_lag1[i] <- my_data$YP_hat[i-1]
     ...
     my_data$YP_hat[i]=(1+(my_data$Pi_hat[i]/100))*my_data$Y_hat[i]
...
# Simulate a persistence parameter from the Beta distribution
    ...
    # Ensure the calculate_ema_4th_order function is defined to return a list with expected_next
      my_data$expected_C_hat_next[i] <- calculate_next_period_with_average_diff(my_data$C_hat[(i-5):(i-1)])
        my_data$expected_I_hat_next[i] <- calculate_next_period_with_average_diff(my_data$I_hat[(i-5):(i-1)])
        my_data$expected_Q_hat_next[i] <- calculate_next_period_with_average_diff(my_data$Q_hat[(i-5):(i-1)])
        my_data$expected_W_hat_next[i] <- calculate_next_period_with_average_diff(my_data$W_hat[(i-5):(i-1)])
        my_data$expected_Pi_hat_next[i] <- calculate_next_period_with_average_diff(my_data$Pi_hat[(i-5):(i-1)])
        my_data$expected_R_hat_next[i] <- calculate_next_period_with_average_diff(my_data$R_hat[(i-5):(i-1)])
         my_data$expected_L_hat_next[i] <- calculate_next_period_with_average_diff(my_data$L_hat[(i-5):(i-1)])
 ...
 # Calculating individual parts of the equation
  ...
  # Break down the equation into parts as before
   ...
   my_data$W_hat[i] <- part1w + part2w + part3w - part4w + part5w - complex_part + my_data$epsilon_W[i]
     ...
     # Debugging: print each part's value
    
   
   print(paste("Iteration:", i))
    print(paste("Part1:", part1w))
    print(paste("Part2:", part2w))
    print(paste("Part3:", part3w))
    print(paste("Part4:", part4w))
   print(paste("Part5:", part5w))
    print(paste("Part6:", adjustment_factor))
   print(paste("complex:", complex_part))
   
   
   
    print(paste("W_hat[i]:", my_data$W_hat[i]))
