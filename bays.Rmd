...
part3 <- ((sigma_c - 1) / (sigma_c * (1 + lambda_w) * (1 + h))) * ( my_data$expected_L_hat_next[i-1]- my_data$L_hat[i])
...
# Combine parts
 ...
 # Calculating individual parts of the equation
  ...
  # Break down the equation into parts as before
    ...
    complex_part <- adjustment_factor * (my_data$W_hat_lag1[i] - (sigma_L * my_data$L_hat[i]) - ((1 / (1 - h) * (my_data$C_hat[i] - (h*my_data$expected_C_hat_next[i-1]))) * my_data$epsilon_W[i])) + my_data$epsilon_L[i]
    ...
    print(paste("Part1:", part1))
    print(paste("Part2:", part2))
    print(paste("Part3:", part3))
    print(paste("Part4:", part4))
   print(paste("Part5:", part5))
   print(paste("adjustment :", adjustment_factor))
    print(paste("complex_part:", complex_part))
