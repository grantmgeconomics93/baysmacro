...
my_data <- data.frame(
   ...
   time = 1:num_rows,
    ...
    K_hat = rep(350, num_rows),
    r_k_star = rep(-500, num_rows),
    
    Q_hat = rep(14, num_rows),
    pi_bar = rep(3, num_rows),
...
my_data$w_hathours_shocked[t]+wage_growth_shocked[t]-(1+psi)*ffr_shocked[t]

        
  # Output gap equation
my_data$Q_hat[i] = -(my_data$R_hat[i] - my_data$Pi_hat_plus_1[i]) +
       ((1-tao) / (1 - tao + r_k_star)) * my_data$Q_hat[i-1] +
...
data_list <- list(
  ...
  expected_C_hat_next = my_data$C_hat_plus_1[1:T],
  expected_I_hat_next = my_data$I_hat_plus_1[1:T],
  expected_Y_hat_next = my_data$Y_hat_plus_1[1:T],
  expected_Pi_hat_next = my_data$Pi_hat_plus_1[1:T],
  expected_R_hat_next = my_data$R_hat_plus_1[1:T],
  expected_L_hat_next = my_data$L_hat_plus_1[1:T],
  expected_W_hat_next = my_data$w_hat_plus_1[1:T],
...
parameters {
 ...
 for (t in shock_time:min(shock_time + 4, T)) {
  ...
  for (t in (shock_time + 1):T) {
    ...
    q_shocked[t] = -(ffr_shocked[t] - inflation_shocked[t-1]) + ((1-tao) / (1 - tao + ffr_shocked[t])) * q_shocked[t-1] +(ffr_shocked[t] / (1 - tao + ffr_shocked[t])) * ffr_shocked[t-1] + epsilon_Q[t];
...
model {
 ...
 q[t] ~ normal(-(ffr_vector[t] - inflation_vector[t]) +
    ...
    (r_k_star[t] / (1 - tao + ffr_vector[t])) * ffr_vector[t-1]+epsilon_Q[t],5.701086);
    
    investment_growth_vector[t] ~ normal((1 / (1 + Beta)) * investment_growth_vector[t-1] +(Beta / (1 + Beta)) *expected_I_hat_next[t]+((1/phi_uppercase)/(1+Beta))*q[t]+epsilon_I[t],17.244);
