...
# Fit a polynomial regression model of a specified order
poly_order <- 3 # Change this to the desired polynomial order
poly_model <- lm(Output_growth ~ poly(row_num, poly_order, raw = TRUE), data = final)

# Predict using the polynomial model
final <- final %>%
  mutate(potential_gdp = predict(poly_model))

# Plot the data
ggplot(final, aes(x = row_num)) +
  geom_line(aes(y = Output_growth, color = "Actual Output Growth")) +
  geom_line(aes(y = potential_gdp, color = "Polynomial Fit")) +
  labs(title = "Actual Output Growth vs. Polynomial Fit",
       x = "Time",
       y = "Output Growth",
       color = "Legend") +
  theme_minimal() +
  scale_color_manual(values = c("Actual Output Growth" = "red", "Polynomial Fit" = "blue"))
...
parameters {
 ...
 for (t in shock_time:min(shock_time + 4, T)) {
  inflation_shocked[t] = inflation_shocked[t] + 12;
}


  // Propagate the shock through other series
  for (t in (shock_time + 1):T) {
  ffr_shocked[t] = inflationtaget_shocked[t] +
                     rho * (ffr_shocked[t-1] - inflationtaget_shocked[t-1]) +
                     (1 - rho) * (r_pi * (inflation_vector[t-1] -  inflationtaget_shocked[t-1]) + 
                                  r_y * (output_growth_vector[t-1])) +
                     Ry_delta * ((inflation_vector[t] -  inflationtaget[t]) - (inflation_vector[t-1]-inflationtaget_shocked[t-1])) +
                     r_y * ((output_growth_vector[t]) - (output_growth_vector[t-1])) +
                     epsilon_R[t];
    real part1 = (h / (1 + h)) * consumption_growth_shocked[t-1]; 
    real part2 = (1 / (1 + h)) * consumption_growth_shocked[t-1]; 
    real part3 = ((sigma_c - 1) / (sigma_c * (1 + lambda_w) * (1 + h))) * (hours_shocked[t-1] - hours_shocked[t]);
    ...
    investment_growth_shocked[t] = (1 / (1 + Beta)) * investment_growth_shocked[t-1] + (Beta / (1 + Beta)) * investment_growth_shocked[t-1] + ((1/phi_uppercase)/(1+Beta)) * q_shocked[t] + epsilon_I[t];
    ...
    real part1w = (Beta / (1 + Beta)) * wage_growth_shocked[t];
    real part2w = (1 / (1 + Beta)) * wage_growth_shocked[t-1];
    real part3w = (Beta / (1 + Beta)) * (inflation_shocked[t-1] - inflationtaget_shocked[t]);
    real part4w = ((1 + (Beta * gamma_w)) / (1 + Beta)) * (inflation_shocked[t-1] - inflationtaget_shocked[t]);
    real part5w = (gamma_w / (1 + Beta)) * (inflation_shocked[t-1] - inflationtaget_shocked[t]);
...
model {
  ...
  for (t in 2:T)  {
  ffr_shocked[t] ~ normal(
      inflationtaget[t] +
      rho * (ffr_shocked[t-1] - inflationtaget[t-1]) +
      (1 - rho) * (r_pi * (inflation_vector[t-1] -  inflationtaget[t-1]) + 
                   r_y * (output_growth_vector[t-1])) +
      Ry_delta * ((inflation_vector[t] - inflationtaget_shocked[t]) - (inflation_vector[t-1] -  inflationtaget[t-1])) +
      r_y * ((output_growth_vector[t]) - (output_growth_vector[t-1])) +
      epsilon_R[t],
      0.9217559);
...
// Breaking down the calculation into parts
 real parta = (Beta / (1 + Beta * gamma_pi)) * (inflation_vector[t-1]-inflationtaget[t]);
 real partb = (gamma_pi / (1 + Beta * gamma_pi)) * (inflation_vector[t-1] -inflationtaget[t]) ;
 real partc =  (1/(1 - Beta * gamma_pi))* (((1-Beta*zeta_ps)*(1-zeta_ps))/zeta_ps)*(alpha*ffr_vector[t]+((1-alpha)*wage_growth_vector[t])-epsilon_pitwo[t]-(1-alpha)*gamma_w);
 real parte =epsilon_PI[t];
// Combine parts
  inflation_vector[t]~  normal(parta + partb + partc + parte +  inflationtaget[t],0.11154431);
   
     real part1w = (Beta / (1 + Beta)) * wage_growth_vector[t];
    real part2w = (1 / (1 + Beta)) * wage_growth_vector[t-1];
    real part3w = (Beta / (1 + Beta)) * (inflation_vector[t-1] - inflationtaget[t]);
    real part4w = ((1 + (Beta * gamma_w)) / (1 + Beta)) * (inflation_vector[t-1] - inflationtaget[t]);
    real part5w = (gamma_w / (1 + Beta)) * (inflation_vector[t-1] -inflationtaget[t]);
    real adjustment_factor = (1/(1+Beta))*((1 - (Beta * zeta_ws) * (1 - zeta_ws)) / (1 + ((1 - lambda_w) * sigma_L) / lambda_w) * zeta_ws);
