...
my_data$Q_hat[i] = -(my_data$R_hat[i] - my_data$Pi_hat_plus_1[i]) +
 ...
 my_data$K_hat[i]=(1-tao)*(my_data$L_hat[i-1]+my_data$w_hat[i-1]-(1+psi)*my_data$R_hat[i-1])+tao*my_data$I_hat_lag1[i]
...
parameters {
 vector[T] expected_Pi_hat_next;
  ...
  real<lower=0.16, upper=0.4> gy; // Computed from 1-Cy-Invy, so range needs to be calculated based on those values
...
// Apply a 30% decrease in output growth at the specified time
 for (t in shock_time:min(shock_time + 4, T)) {
  ...
  ffr_shocked[t] = inflation_shocked[t-1] +
                 ...
                 0.5 * (inflation_shocked[t-1] - inflationtarget_shocked[t-1]) + inflationtarget_shocked[t-1] + epsilon_R[t];

consumption_growth_shocked[t] = (h / (1 + h)) * consumption_growth_shocked[t-1] +
                                (1 / (1 + h)) * expected_C_hat_next[t] +
                                ((sigma_c - 1) / (sigma_c * (1 + lambda_w) * (1 + h))) * (hours_shocked[t-1] - expected_L_hat_next[t]) -
...
output_growth_shocked[t] = (1 - (tao * Ky) - gy) * consumption_growth_shocked[t] +
                           (tao * Ky) * investment_growth_shocked[t] + gy * epsilon_G[t];

inflation_shocked[t] = (Beta / (1 + Beta * gamma_pi)) * (expected_Pi_hat_next[t] - inflationtarget_shocked[t]) +
                       (gamma_pi / (1 + Beta * gamma_pi)) * (inflation_shocked[t-1] - inflationtarget_shocked[t]) +
                       (1 / (1 - Beta * gamma_pi)) * (((1 - Beta * zeta_ps) * (1 - zeta_ps)) / zeta_ps) *
                       (alpha * ffr_shocked[t] + ((1 - alpha) * wage_growth_shocked[t]) - epsilon_pitwo[t] - (1 - alpha) * gamma_w) +
                       epsilon_PI[t] + inflationtarget_shocked[t];

wage_growth_shocked[t] = (Beta / (1 + Beta)) * expected_W_hat_next[t] +
                         (1 / (1 + Beta)) * wage_growth_shocked[t-1] +
                         (Beta / (1 + Beta)) * (inflationtarget_shocked[t-1] - inflation_shocked[t]) -
                         ((1 + (Beta * gamma_w)) / (1 + Beta)) * (inflationtarget_shocked[t-1] - inflation_shocked[t]) -
                         (1 / (1 + Beta)) * ((1 - (Beta * zeta_ws) * (1 - zeta_ws)) / (1 + ((1 - lambda_w) * sigma_L) / lambda_w) * zeta_ws) *
                         (wage_growth_shocked[t-1] - (sigma_L * hours_shocked[t]) -
                          (1 / (1 - h)) * (consumption_growth_shocked[t] - (h * consumption_growth_shocked[t-1]))) +
                         epsilon_L[t] + epsilon_W[t];
...
model {
  ...
  mu_C_hat ~ normal(0.5415428, sigma_C_hat);
  mu_I_hat ~ normal(0.4016593, sigma_I_hat);
  mu_R_hat ~ normal(1.596101, sigma_R_hat);
  mu_W_hat ~ normal(0.3624955, sigma_W_hat);
  mu_L_hat ~ normal(-154.7743, sigma_L_hat);
  ...
  expected_W_hat_next[1] ~ normal(mu_W_hat, sigma_W_hat);
  ...
  for (t in 2:T) {
    ...
    expected_W_hat_next[t] ~ normal(expected_W_hat_next[t-1], sigma_W_hat);
    ...
    ffr_vector[t] ~ normal(inflation_vector[t-1] + 
                   ...
                   (0.5 * (inflation_vector[t-1] -  inflationtarget[t-1])) + 
                    inflationtarget[t] + epsilon_R[t],0.9289912);
    ...
    consumption_growth_vector[t] ~ normal(part1 + part2 + part3 + part4 + part5,0.7461217);
 ...
 q[t] ~ normal(-(ffr_vector[t] - inflation_vector[t]) +
   ...
   kapital[t] ~ normal((1 - tao) * kapital[t - 1] + tao * investment_growth_vector[t] + tao *epsilon_I[t],17.22238 );

    
   hours_vector[t]~ normal(-wage_growth_vector[t] + (1+psi)*ffr_vector[t]+ kapital[t-1],2.57808);
   
   output_growth_vector[t] ~ normal((1 - (tao*Ky) - gy) * consumption_growth_vector[t] + (tao*Ky) * investment_growth_vector[t] + gy *epsilon_G[t],0.7320224);
      
// Breaking down the calculation into parts
 real parta = (Beta / (1 + Beta * gamma_pi)) * (expected_Pi_hat_next[t]-inflationtarget[t]);
 real partb = (gamma_pi / (1 + Beta * gamma_pi)) * (inflation_vector[t-1] -inflationtarget[t]) ;
...
// Combine parts
  inflation_vector[t]~  normal(parta + partb + partc + parte +  inflationtarget[t],0.4543148);
   
     real part1w = (Beta / (1 + Beta)) *expected_W_hat_next[t] ;
    real part2w = (1 / (1 + Beta)) * wage_growth_vector[t-1];
    real part3w = (Beta / (1 + Beta)) * (inflationtarget[t-1] - inflation_vector[t]);
    real part4w = ((1 + (Beta * gamma_w)) / (1 + Beta)) * (inflationtarget[t-1] - inflation_vector[t]);
    real part5w = (gamma_w / (1 + Beta)) * (inflationtarget[t-1] -inflation_vector[t]);
    ...
    wage_growth_vector[t] ~ normal(part1w + part2w + part3w - part4w + part5w - complex_part + epsilon_W[t], 0.676194);
  }
   }
...
plot_shocked_variable <- function(original, mean_shocked, lower_ci, upper_ci, variable_name) {
  ...
  ymin <- min(c(original, mean_shocked), na.rm = TRUE) - 5
  ymax <- max(c(original, mean_shocked), na.rm = TRUE) + 5
...
# Generate the list of plots with dynamic y-axis limits
plot_list <- list(
  plot_shocked_variable(data_list$output_growth_vector, mean_output_growth_shocked, lower_credible_interval_output, upper_credible_interval_output, "Output Growth"),
  plot_shocked_variable(data_list$consumption_growth_vector, mean_consumption_growth_shocked, lower_credible_interval_consumption, upper_credible_interval_consumption, "Consumption Growth"),
  plot_shocked_variable(data_list$investment_growth_vector, mean_investment_growth_shocked, lower_credible_interval_investment, upper_credible_interval_investment, "Investment Growth"),
