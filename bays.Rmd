...
```
```{r}
# Initialize the dataframe with the first 4 rows set to 1
my_data <- data.frame(
    time = 1:104,
    C_hat = rep(10000, 104),
    L_hat=rep(10000,104),
    ...
    Q_hat = rep(10000,104),
     pi_bar=rep(1,104),
    YP_hat=my_data$Pi_hat/my_data$W_hat,
    epsilon_C = rep(1, 104),
    epsilon_Q = rep(1, 104),
     epsilon_I = rep(1, 104),
    epsilon_W = rep(1, 104),
    ...
    expected_RK_hat_next=rep(10000,104),
     ...
     expected_W_hat_next=rep(10000,104),
    ...
    C_hat_lag1 = rep(10000, 104),  # Lagged values for the first row would be NA
    I_hat_lag1 = rep(10000, 104),# Lagged values for the first row would be NA
    Q_hat_lag1 = rep(10000, 104),
    W_hat_lag1=rep(10000,104),
...
for (i in 5:104) {
    ...
    my_data$YP_lag1[i] <- my_data$YP_hat[i-1]
    
    
     # Generate new shock value for epsilon_c for the current row
...
# Simulate a persistence parameter from the Beta distribution
     ...
     epsilon_Q<- rnorm(1, mean = 0, sd = sigma_e_q)
      ...
      my_data$epsilon_Pi[i]=epsilon_p
       ...
       my_data$epsilon_pitwo[i] <- rho_g * my_data$epsilon_pitwo[i-1] + eta_g
        epsilon <- rnorm(1, mean = 0, sd = sigma_e_w)
       my_data$epsilon_W[i]=epsilon_W
    ...
    # Now update 'C_hat' based on the updated logic
    # Assuming h, lambda_w, phi are defined and calculated correctly
    my_data$C_hat[i] <- (h / (1 + h))* my_data$C_hat_lag1[i] +
                        (1 / (1 + h)) * my_data$expected_C_hat_next[i-1] +
                        ((sigma_c - 1) / (sigma_c * (1 + lambda_w) * (1 + h))) * (my_data$I_hat[i] - my_data$expected_I_hat_next[i-1]) -
                        (1 - h) / ((1 + h) * sigma_c) * (my_data$R_hat[i] - my_data$expected_Pi_hat_next[i-1]) +
...
my_data$Q_hat[i] = -(my_data$R_hat[i] - my_data$expected_Pi_hat_next[i-1]) +
        ...
        (my_data$R_k[i] / (1 - tao + r_k_star)) * my_data$expected_R_hat_next[i-1] + my_data$epsilon_Q[i]
...
# Combine parts
 my_data$Pi_hat[i]<- parta + partb + partc + parte + my_data$pi_bar[i] 
    ...
    my_data$R_hat[i] = my_data$pi_bar[i] + 
              rho * (my_data$Pi_bar_lag1[i] - my_data$Pi_bar_lag1[i]) +
    ...
    part3 <- (Beta / (1 + Beta)) * (my_data$expected_Pi_hat_next[i-1] - 4)
    part4 <- - ((1 + (Beta * gamma_w)) / (1 + Beta)) * (my_data$Pi_hat_lag1[i] - 4)
    part5 <- (gamma_w / (1 + Beta)) * (my_data$expected_Pi_hat_next[i-1] - 4)
...
for (i in 5:104) {
    ...
    eta_c[i] <- rnorm(1, mean = 0, sd = sigma_eta_c)
    my_data$epsilon_c[i] <- rho_c * my_data$epsilon_c[i-1] + eta_c[i]
