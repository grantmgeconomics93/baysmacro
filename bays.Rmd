...
inflation_shocked[t] = (Beta / (1 + Beta * gamma_pi)) * (expected_Pi_hat_next[t] - inflationtarget_shocked[t]) +
 ...
 // Part 1: Expected future wage growth
    ...
    real complex_term = adjustment_factor *  (wage_growth_shocked[t-1] - (sigma_L * hours_shocked[t]) -  (1 / (1 - h)) * (consumption_growth_shocked[t] - (h * consumption_growth_shocked[t-1]+epsilon_L[t])));

    // Error terms
    real error_term = epsilon_W[t];

    // Final wage growth shocked equation
    wage_growth_shocked[t] = part1_wage + part2_wage + part3_inflation - part4_inflation_wage - complex_term + error_term;
...
```{r}
fit <- sampling(model, data = data_list, algorithm = "NUTS", iter = 2000, chains = 2)
