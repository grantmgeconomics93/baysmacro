...
parameters {
    ...
    real<lower=0.55, upper=0.95> Beta; // Given mean is 0.025, the range would be truncated because Beta cannot be negative
  ...
  real<lower=0.5, upper=1> gamma_w; // Mean=0.763, so range is 0.763±0.5, upper bound truncated at 1
  ...
  real<lower=0.5, upper=1.2> phi_uppercase; // Mean=1/6.771, requires calculating the actual value and then ±0.5, truncating bounds as necessary
  ...
  real<lower=21.75, upper=22.25> Invy;
...
// Breaking down the calculation into parts
 real parta = (Beta / (1 + Beta * gamma_pi)) * (inflation_vector[t-1]-pi_bar[t]);
 real partb = (gamma_pi / (1 + Beta * gamma_pi)) * (inflation_vector[t-1] -pi_bar[t]) ;
...
// Combine parts
  inflation_vector[t]~  normal(parta + partb + partc + parte + pi_bar[t],0.6454431);
   
     real part1w = (Beta / (1 + Beta)) * wage_growth_vector[t];
    real part2w = (1 / (1 + Beta)) * wage_growth_vector[t-1];
    real part3w = (Beta / (1 + Beta)) * (inflation_vector[t-1] - pi_bar[t]);
    real part4w = ((1 + (Beta * gamma_w)) / (1 + Beta)) * (inflation_vector[t-1] - pi_bar[t]);
    real part5w = (gamma_w / (1 + Beta)) * (inflation_vector[t-1] - pi_bar[t]);
...
```{r}
Warning messages:
1: There were 18220 divergent transitions after warmup. See
...
partb<- (gamma_pi / (1 + Beta * gamma_pi)) * (inflation_vector[t] -pi_bar[t]) # Check if this should be [i] or [i-1]
partc <-  (1/(1 - Beta * gamma_pi))* (((1-Beta*zeta_ps)*(1-zeta_ps))/zeta_ps)*(alpha*ffr_vector[t]+((1-alpha)*wage_growth_vector[t])-epsilon_pitwo[t]-(1-alpha)*gamma_w) 
parte <- epsilon_PI[i]
# Combine parts
 my_data$Pi_hat[i]<- normal(parta + partb + partc + parte + my_data$pi_bar[i],) 
