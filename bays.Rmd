...
```
```{r}
# Extract parameter names again to confirm
param_names <- dimnames(params)[[3]]
print(param_names)
# Using ggplot2 for a custom trace plot if 'bayesplot' fails
if (exists("beta_data")) {
    library(ggplot2)
    beta_df <- data.frame(Iteration = rep(1:nrow(beta_data), ncol(beta_data)),
                          Beta = as.vector(t(beta_data)),
                          Chain = rep(1:ncol(beta_data), each = nrow(beta_data)))
    ggplot(beta_df, aes(x = Iteration, y = Beta, color = as.factor(Chain))) +
        geom_line() +
        labs(title = "Trace Plot for Beta", x = "Iteration", y = "phi") +
        theme_minimal()
}
