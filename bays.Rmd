...
// Fiscal inflation with shock
    ...
    R_b_shocked[t-1] = (rho_M / R_short_term) * P_bond_shocked[t] - P_bond_shocked[t-1];
    
    // Government debt with shock
    government_debt_shocked[t] = inv_beta * government_debt_shocked[t-1]
                                 + b_param * inv_beta * (R_b_shocked[t-1] - output[t] + output[t-1] - inflation[t])
                                 - tau_shocked[t];
...
// Mean=0.5, SD=0.15 implies alpha=beta
 ...
 inflation_vector[t-1] +
                 0.5 * ( output_gap) +
                 0.5 * (inflation_vector[t-1] - inflationtaget[t-1]) + inflationtarget[t] + epsilon_R[t] ,
  0.9289912
);
...
// Combine parts
  inflation_vector[t]~  normal(kappa * (output_growth[t] - potential_gd[t]) + beta * expected_Pi_hat_next[t],0.4543148);
    ...
    R_b[t-1] = (rho_M / R_short_term[t-1]) * P_bond[t] - P_bond[t-1];
