...
calculate_next_period_with_average_diff <- function(data) {
  ...
  next_period <- tail(data, 1)+(.18*avg_diff) 
...
my_data <- data.frame(
    ...
    W_hat = rep(3, 104),
    I_hat = rep(35, 104),
     C_hat_lag1 = rep(333, 104),  # Lagged values for the firstrhow would be NA
    I_hat_lag1 = rep(35, 104),
     expected_I_hat_next=rep(35,104),
     expected_W_hat_next=rep(3,104),
    K_hat=rep(0,104),
    Pi_hat=rep(1,104),
    Y_hat=rep(2000,104),
    R_hat = rep(7,104),
    ...
    expected_K_hat_next=rep(0,104),
    expected_L_hat_next=rep(7,104),
    expected_C_hat_next=rep(333,104),
    expected_Q_hat_next = rep(10, 104),
    expected_Pi_hat_next=rep(1,104),
    expected_R_hat_next = rep(7, 104),
     expected_Y_hat_next = rep(2000, 104),
   # Lagged values for the firstrhow would be NA
    Q_hat_lag1 = rep(1, 104),
    W_hat_lag1=rep(3,104),
     R_hat_lag1 = rep(7,104),
    L_hat_lag1 = rep(1,104),
    Y_hat_lag1=rep(1,104),
    YP_hat_lag1=rep(1,104),
    Pi_bar_lag1=rep(1,104),
     Pi_hat_lag1=rep(1,104),
    K_hat_lag1 = rep(0,104)
)
...
# Combine parts
 ...
 # Calculating individual parts of the equation
  ...
  # Break down the equation into parts as before
   ...
   my_data$W_hat[i] <- part1w + part2w + part3w - part4w + part5w - (complex_part/120) + my_data$epsilon_W[i]
