...
parameters {
 ...
 for (t in shock_time:min(shock_time + 4, T)) {
  inflation_shocked[t] = inflation_shocked[t] + 2.6;
}
  ...
  for (t in (shock_time + 1):T) {
    ffr_shocked[t] = inflation_shocked[t-1] + 
                   0.5 * (output_growth_shocked[t-1] - potential_gdp[t-1]) + 
                   0.5 * (inflation_shocked[t-1] -  inflationtaget[t-1]) + 
                    inflationtaget[t-1] + epsilon_R[t];
...
model {
  ...
  for (t in 2:T)  {
    ffr_shocked[t] ~ normal(inflation_shocked[t-1] + 
                   0.5 * (output_growth_shocked[t-1] - potential_gdp[t-1]) + 
                   0.5 * (inflation_shocked[t-1] -  inflationtaget[t-1]) + 
